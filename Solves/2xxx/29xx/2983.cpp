...
public:
    vector<bool> canMakePalindromeQueries(string s, vector<vector<int>> &queries)
    {
        ...
        for (auto &q : queries)
        {
            ...
            if (min(a1, d2) && psd[min(a1, d2)] || n / 2 > max(b1, c2) && psd[n / 2] - psd[max(b1, c2)] || d2 > b1 && psd[d2] - psd[b1] || a1 > c2 && psd[a1] - psd[c2])
            {
                ans.push_back(false);
            }
            else
            {
                ...
                if (a1 > d2)
                    ix1 -= psc[min(a1, c2)] - psc[d2];
                if (c2 > b1)
                    ix1 -= psc[c2] - psc[max(b1, d2)];
                if (c1 > b2)
                    ix2 -= psc[min(c1, a2)] - psc[b2];
                if (a2 > d1)
                    ix2 -= psc[a2] - psc[max(d1, b2)];
